...
9. **The pipeline transports bytes; it does not judge.** It may fetch, store, index, deduplicate, drop noise fields, and rank candidates by crude signals. It may **never** paraphrase, summarise, translate, or "clean up" a source sentence. What it hands over must be the raw API payload, byte-identical to what the source returned.
