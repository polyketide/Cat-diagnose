...
### 12a. The two conclusions it finally landed — recorded as pointers, not restated as claims

§12 records how the fact-check went wrong; on its own it reads as a failure with no landing. Once the documents were read and the currency scan done, the episode produced two solid conclusions, and they belong here as the landing. **They are recorded as one-line outcomes plus a pointer to where the evidence lives — deliberately *not* restated in full, because the SOP is not a second source of truth for medical content and a copied conclusion is one that drifts (§3d, §3g, §3h). Each lives, verbatim-sourced, in its own knowledge-base entry; cite that file, do not paraphrase this paragraph. Numbers in particular stay out of here on purpose.**

1. **Blood-transfusion compatibility — the one dangerous error in the announcement.** The claim that any healthy cat's blood can simply be given is wrong and can kill on the first unit: cats carry naturally occurring alloantibodies, only AB-matched blood is safe, and — the currency layer the 1990s reviews lack — AB-matching is *necessary but not sufficient* (the Mik antigen, and non-AB crossmatch incompatibility common enough in transfusion-naive cats to matter on a first transfusion), so the recipient is typed **and** crossmatched before the first unit. Full evidence, verbatim: `knowledge-base/blood-types-and-transfusion-compatibility.md`. **Lesson carried:** the single place the community was confidently wrong was the single place the decision was irreversible — screen the highest-stakes claim first, not the most conspicuous one.

2. **Chemotherapy-protocol currency — the reassuring half, made honest.** The group's chemo digest was faithful to the textbook backbone (that was the reversal in §12 step 3); a forward read confirmed the core first-line protocols still stand and surfaced two 2025 options the text predates — abdominal radiation for the small-cell/CIE end, and lomustine + corticosteroids for CNS lymphoma. Full evidence, verbatim: `knowledge-base/feline-lymphoma-treatment-currency.md`, with reference doses held to §3h. **Lesson carried:** "faithful to the textbook" and "current" are two separate verdicts; deliver both, and never let one stand in for the other.

**Why an outcome belongs in a process document.** Not as medical reference — that is the corpus's job — but because a recorded failure without its resolution teaches only half of what happened. The pair above closes §12, and recording them as *pointers into the corpus* rather than restated claims is itself the discipline the failure taught: the evidence has exactly one home, and this document points at it rather than becoming a rival copy.
